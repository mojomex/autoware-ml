...
def test_rotary_span_is_the_largest_multiple_of_six(head_dim: int, expected: int) -> None:
    """Divisibility by three is not enough: an odd chunk width mispairs the halves."""
    assert rotary_span(head_dim) == expected
...
def test_litept_export_contract_drops_the_serialization_it_never_reads() -> None:
    """Attention-free stages contribute no order tensors, and stage 0 no base code."""
    contract = EncoderExportContract.from_encoder(build_litept_encoder(), include_cluster=False)

    assert contract.needs_base_code is False
    assert contract.input_names == [
        "grid_coord",
        "feat",
...
def test_ptv3_export_contract_is_unchanged_by_the_refactor() -> None:
    """The PTv3 contract still declares the base code and every order tensor."""
    contract = EncoderExportContract.from_encoder(build_ptv3_encoder(), include_cluster=False)

    assert contract.needs_base_code is True
    assert contract.input_names == [
        "grid_coord",
        "feat",
        "serialized_code",
...
@REQUIRES_SPARSE_CUDA
def test_litept_split_export_declares_and_consumes_the_reduced_contract() -> None:
    """Every declared encoder input is consumed, so the tracer prunes nothing."""
    model = build_litept_seg_model().cuda().eval()
    batch = move_batch_to_device(build_inputs(), torch.device("cuda"))
    ...
    assert not any(
        name.startswith("serialized_pooling_0_serialized")
        for name in encoder_spec.input_param_names
    )
...
def test_litept_monolithic_export_runs_on_its_declared_inputs() -> None:
    """The single-graph export keeps clusters but still drops the unused order tensors."""
    model = build_litept_seg_model().cuda().eval()
    batch = move_batch_to_device(build_inputs(), torch.device("cuda"))

    spec = model.build_export_spec(batch)

    assert "serialized_code" not in spec.input_param_names
    assert "serialized_pooling_0_cluster" in spec.input_param_names
    assert "serialized_pooling_0_serialized_order" not in spec.input_param_names
